...
* [ ] propagate failures from `rbenv versions --bare` instead of treating an empty version list as success;
* [ ] propagate `find` failures in `--separate` mode instead of succeeding when test discovery fails;
